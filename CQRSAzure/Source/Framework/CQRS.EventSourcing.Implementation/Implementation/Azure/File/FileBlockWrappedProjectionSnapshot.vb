...
Namespace Azure.File
    ...
    <Serializable()>
    <DataContract()>
    Public NotInheritable Class FileBlockWrappedProjectionSnapshot
        Implements IProjectionSnapshot

        <IgnoreDataMember()>
        <NonSerialized()>
        Private m_formatter As IFormatter
        ...
        Private ReadOnly m_values As New List(Of FileBlockWrappedProjectionSnapshotProperty)
        ...
        Public Sub WriteToBinaryStream(ByVal stream As System.IO.Stream)
            ...
            If (m_formatter Is Nothing) Then
                m_formatter = New BinaryFormatter()
            End If
            m_formatter.Serialize(stream, Me)

        End Sub

        Public Shared Function FromBinaryStream(ByVal binaryStream As System.IO.Stream,
                                                Optional ByVal formatter As IFormatter = Nothing) As FileBlockWrappedProjectionSnapshot
        ...
        Public Sub New(ByVal projectionToWrap As IProjectionSnapshot)
            ...
            For Each projectionValue In projectionToWrap.Values
                m_values.Add(New FileBlockWrappedProjectionSnapshotProperty(projectionValue))
            Next
    ...
    <Serializable()>
    <DataContract()>
    Public NotInheritable Class FileBlockWrappedProjectionSnapshotProperty
